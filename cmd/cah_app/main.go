...
const expansionsDir = "web/expansions/"
...
func run() {
	printRunningDir()
	sqlite.InitDB("internal/db/database.sqlite3")
...
func populateCards(cardUC cah.CardUsecases) {
	files, err := ioutil.ReadDir(expansionsDir)
	if err != nil {
	...
	for _, f := range files {
		if f.IsDir() {
			...
			err := cardUC.CreateFromFolder(expansionsDir+f.Name(), f.Name())
			if err != nil {
